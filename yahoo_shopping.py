...
def get_search_url(query_keyword):
    ...
    base_url = 'https://shopping.yahoo.co.jp/search?first=1&tab_ex=commerce&fr=shp-prop&oq=&aq=&mcr=cebc49a038126e769c49059c75e78462&ts=1614588753&sc_i=shp_pc_top_searchBox&p='
...
def feature_product_details(url):
    ...
    product_title = [i.getText().split('\n') for i in url.find_all("div", attrs={"class":"LoopList__item"})]
    product_price = [i.getText().split('\n') for i in url.find_all("div", attrs={"class":"_2jgEMnhQANtx"})]
    print(product_title)
    print(product_price)
    product_lists = []
